...
class NicknameFormatCommand extends Command {
  constructor (client) {
    super(client, {
      ...
      description: "`<format>` Set the format RoVer will use when setting users' nicknames. Available replacements are %SMARTNAME%, %USERNAME%, %DISPLAYNAME%, %RANK%, %USERID%, %SERVER%, %DISCORDNAME%, and %DISCORDID%. Example:` %USERNAME% - (%USERID%)`. Default `%USERNAME%`.",
