...
module.exports = class WhoisCommand extends Command {
  ...
  async fn(msg, args) {
    ...
    let data = {}
    if (member) {
      // If the member specified exists,
      if (member.user.bot) return
      const editMessage = await msg.reply(
        `:mag: Looking up ${member.displayName.replace(/@/g, "")}`,
      )
      ...
      try {
        ...
        return editMessage.edit(
          "An error occured while fetching that user's data.",
        )
      }
      ...
      if (data.status === "ok") {
        ...
        try {
          ...
          pastNamesData.data.forEach((oldname) => {
            pastNames += `, ${oldname.name}`
          })
        ...
        const embed = {
          ...
          description: bio,
          fields: [{ name: "Join Date", value: joinDate, inline: true }],
        }
        ...
        if (nicknameGroup) {
          const userGroups = await DiscordServer.getRobloxMemberGroups(
            data.robloxId,
          )

          const serverGroup = userGroups.find(
            (group) => group.group.id === parseInt(nicknameGroup),
          )

          let isAlly = false
          if (!serverGroup) {
            isAlly = await VirtualGroups._Relationship(
              { id: data.robloxId },
              parseInt(nicknameGroup),
              DiscordServer,
              "allies",
            )
          }
        ...
        try {
          const response = await request({
            uri: `https://scriptinghelpers.org/resources/get_profile_by_roblox_id/${encodeURIComponent(
              data.robloxId,
            )}`,
            simple: false,
            resolveWithFullResponse: true,
        ...
        if (devforumData) {
          ...
          embed.fields.push({
            name: "DevForum",
            value: `[Profile Link](https://devforum.roblox.com/u/${
              devforumData.username
